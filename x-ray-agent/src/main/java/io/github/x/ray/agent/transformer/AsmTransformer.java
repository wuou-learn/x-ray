package io.github.x.ray.agent.transformer;
...
public class AsmTransformer implements ClassFileTransformer {
    ...
    private final List<? extends XrayHandle> handleList;

    private List<String> staticNameFilterList = Arrays.asList("bycglib", "_asm", "byspringcglib", "_wrappertypehelper");

    public AsmTransformer(List<? extends XrayHandle> handleList) {
    ...
    public byte[] transform(ClassLoader loader, final String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, final byte[] classfileBuffer) throws IllegalClassFormatException {
        if (className == null || staticFilter(className.toLowerCase())) {
            return new byte[0];
        }
        String realClassName = className.replace("/", ".");
        this.handleList.sort(Comparator.comparingInt(XrayHandle::priority));
        for (XrayHandle handle : this.handleList) {
            if (handle.filterClassName(realClassName)) {
                continue;
            }
            return handle.process(realClassName, classfileBuffer, loader);
        }
        return new byte[0];
    }

    private boolean staticFilter(String className) {
        for(String scope : staticNameFilterList) {
